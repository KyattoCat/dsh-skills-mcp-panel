 ...
 *
 * - `GET  /skills-mcp/state`  — every discovered skill and MCP server row.
 * - `POST /skills-mcp/toggle` — flip one row's enablement.
 *
...
async function handleState(ctx, settings, req, res) {
  ...
  try {
    ...
    sendJson(res, 200, {
      ...
      groups: groupsFor(settings, projects),
...
async function handleToggle(ctx, settings, req, res) {
  ...
  try {
    ...
    sendJson(res, 200, {
      ok: true,
      applied: kind === 'mcp' && profile?.patchReload === 'startup' ? 'restart' : 'hot',
      skills,
      mcp: collectMcp(ctx, projects),
      groups: groupsFor(settings, projects),
      profile: profile === undefined
...
/**
 * The groups a page renders, in display order: the global group first, then one
 * per project root, then one per configured extra root. A group with no rows is
 * dropped by the page, so this list is the complete menu rather than a claim
 ...
 */
function groupsFor(settings, projects) {
  const groups = [{ id: GLOBAL_GROUP, kind: 'global', path: null }]
  for (const project of projects) {
    groups.push({ id: projectGroupId(project), kind: 'project', label: basename(project), path: project })
  }
  for (const path of settings.extraRoots) {
    groups.push({ id: `root:${path}`, kind: 'custom', label: basename(path), path })
  }
  return groups
}
