...
/* The header IS the toggle: one button spanning the row, so the whole strip is
   the hit target instead of a chevron to aim at. Every UA button default is
   reset here, because this is a heading that happens to be clickable. */
.dsm-group-head { display: flex; align-items: baseline; gap: 8px; width: 100%; padding: 0 2px 6px;
  border: 0; background: none; cursor: pointer; text-align: left; font-family: inherit; }
.dsm-group-head:hover .dsm-group-label { color: var(--dsw-alias-label-primary, #e6e6e6); }
.dsm-group-head:focus-visible { outline: 1px solid var(--dsw-alias-border-l4, rgba(255,255,255,0.24));
  outline-offset: 2px; border-radius: 6px; }
.dsm-group-chevron { flex: 0 0 auto; align-self: center; color: var(--dsw-alias-label-tertiary, #8a8f98);
  transition: transform 120ms ease; }
.dsm-group-head.is-open .dsm-group-chevron { transform: rotate(90deg); }
...
/* Says why this group is the one that starts open. */
.dsm-group-current { flex: 0 0 auto; padding: 0 5px; border-radius: 6px; font-size: 10px;
  color: var(--dsw-alias-label-secondary, #a9adb6);
...
`
    ...
    function SkillsMcpSection(props) {
      ...
      // Group collapse state by group id. An absent entry means "no opinion
      // yet": the default belongs to the host's pin, so a group the next load
      // discovers still opens the way the page intends instead of inheriting
      // the state of whatever rendered before it.
      ...
      /**
       * Whether one group is expanded: the user's own choice when they made
       * one, else the host's default. A host that flags no current project (an
       * older half) leaves every group open, which is how the page behaved
       * before pinning existed.
       */
      const groupIsOpen = (group, pinnedId) => {
        const chosen = groupOpen[group.id]
        if (chosen !== undefined) return chosen === true
        return pinnedId === undefined || group.id === pinnedId
      }

      const toggleGroup = (id, isOpen) => {
        setGroupOpen((current) => ({ ...current, [id]: !isOpen }))
      }
      ...
      if (state.status === 'loading' || state.status === 'idle') {
        ...
        // host's (the current project first, then global and the rest); a row
        // whose group is missing from the menu still renders, under its own id,
        ...
        // The host flags which group is the current project's; that flag is the
        // whole default-open rule, so the page never has to compare paths.
        const visible = menu.filter(group => byGroup.has(group.id))
        const flagged = menu.find(group => group.current === true)?.id
        // A flagged project that contributes no rows has no group to render, and
        // opening nothing would land the page fully collapsed: the first group
        // that does have rows opens instead. A host that flags nothing keeps the
        // page as it always was, with every group open.
        const pinnedId = flagged === undefined
          ? undefined
          : (visible.some(group => group.id === flagged) ? flagged : visible[0]?.id)
        body.push(h('div', { key: 'groups', className: 'dsm-groups' },
          visible.map(group => {
            const isOpen = groupIsOpen(group, pinnedId)
            ...
            h('button', {
              type: 'button',
              className: isOpen ? 'dsm-group-head is-open' : 'dsm-group-head',
              title: group.path ?? undefined,
              'aria-expanded': isOpen,
              onClick: () => { toggleGroup(group.id, isOpen) },
            },
            h('svg', {
              className: 'dsm-group-chevron',
              viewBox: '0 0 16 16',
              width: 10,
              height: 10,
              'aria-hidden': true,
            ...
            // Right after the name it explains, not stranded at the far edge.
            group.current === true ? h('span', { className: 'dsm-group-current' }, t('currentProject')) : null,
            group.path === null || group.path === undefined
              ? null
              : h('span', { className: 'dsm-group-path' }, group.path),
            h('span', { className: 'dsm-group-count' }, String(groupRows.length))),
            isOpen
              ? h('div', { className: 'dsm-grid' },
                groupRows.map(row => (tab === 'skills' ? skillCard(row) : mcpCard(row))))
              : null)
          })))
      }
    ...
    function apply(ctx) {
      ...
      /**
       * The directory the GUI's current Session runs in. The host pins that
       * project's group with it.
       *
       * Read per request rather than captured at apply time, for two reasons: a
       * Session switch then lands on the next load or refresh, and a client
       * composed without the sessions service (or mounted before it) still
       * renders the panel — the host falls back to its own project. A service
       * whose shape changed is the same non-event as an absent one.
       *
       * @returns an absolute directory, or undefined when no Session is current.
       */
      const currentCwd = () => {
        try {
          const list = ctx.get('sessions')?.list?.getSnapshot()
          const id = list?.current
          const cwd = id === undefined ? undefined : list?.byId?.[id]?.cwd
      ...
      /** The state route, carrying the current project as the host's pin hint. */
      const stateUrl = (cwd) => (cwd === undefined ? STATE_URL : `${STATE_URL}?cwd=${encodeURIComponent(cwd)}`)
      ...
      const load = async () => {
        ...
        try {
          const response = await fetch(stateUrl(currentCwd()), { credentials: 'same-origin', headers: { accept: 'application/json' } })
      ...
      const toggle = async (kind, id, request) => {
        ...
        try {
          const response = await fetch(TOGGLE_URL, {
            ...
            // `cwd` rides along so the answer carries the same pinned menu the
            // page already shows; undefined drops out of the JSON entirely.
            body: JSON.stringify({ kind, id, ...request, cwd: currentCwd() }),
          })
          const payload = await response.json().catch(() => ({}))
